...
string g[MAX][MAX];  //to store entered gmar
string d[MAX];
...
inline void break_g(string a)    //seperates right hand side of entered gmar
{
    int i;
    p=0;
    while(a.length())
...
inline int lchomsky(string a)   //checks if LHS of entered gmar is in CNF
{
...
inline int rchom(string a)   //checks if RHS of gmar is in CNF
{
    if (a.length() == 1 && a[0]>='a' && a[0] <='z')
        return 1;
