...
void main()								// the main function
{
   ...
   printf("\nWelcome to the Genetic Algorithm coded by Loay Al Lusiy = -x^2 + 5 to find the maximum value of the function...\n"); // introduction to the program
    ...
    col=rand()%6;                           	// random column (gene) choosing
     ...
     if (popnext[row].bit[col]==0)          	// invert the bit to 1 if it was 0
      popnext[row].bit[col]=1 ;
       ...
       value=x(popnext[row]);
       printf("\nMutation occured in popnext[%d] bit[%d]:=%d%d%d%d%d%d    value=%dfitness=%d",row,col,popnext[row].bit[5],popnext[row].bit[4],popnext[row].bit[3],
