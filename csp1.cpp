...
struct cityType cities[MAX_CITIES];
struct antType ants[MAX_ANTS];
...
int simulateAnts()
{
    ...
    for(k=0; k<MAX_ANTS; k++)
    {
        ...
        if( ants[k].pathIndex < MAX_CITIES )
           ...
           ants[k].tourLength += dist[ants[k].curCity][ants[k].nextCity];                      //handle last case->last city to first
